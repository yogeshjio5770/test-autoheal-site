...
===================================================
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let e=document.getElementById("ah-feature-view"),t=document.getElementById("ah-feature-footer"),s=document.getElementById("ah-settings-view"),a=document.getElementById("ah-settings-footer"),i=!1;document.getElementById("ah-settings-btn")?.addEventListener("click",()=>{i=!i,i?(e.style.display="none",t.style.display="none",s.style.display="block",a.style.display="flex"):(e.style.display="block",t.style.display="flex",s.style.display="none",a.style.display="none")}),document.getElementById("ah-save-settings-btn")?.addEventListener("click",async()=>{let r=document.getElementById("ah-groq-key-input").value.trim();if(!r)return;let c=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",h=window.AUTOHEAL_SITE_ID||window.location.host,l=document.getElementById("ah-settings-status");l.textContent="Saving...";try{(await(await fetch(`${c}/api/settings`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":h},body:JSON.stringify({settings:{groqKey:r,modelProvider:"groq"}})})).json()).success?l.textContent="Settings saved successfully! \u2705":l.textContent="Error saving settings."}catch{l.textContent="Failed to connect to Master Server."}});let o=document.getElementById("ah-build-btn"),n=document.getElementById("ah-feature-prompt");o.addEventListener("click",()=>{let r=n.value.trim();if(!r)return;n.disabled=!0,o.disabled=!0,o.classList.add("disabled");let c=document.getElementById("ah-btn-loader");c&&(c.style.display="inline-block");let h=document.getElementById("ah-btn-text");h&&(h.textContent="BUILDING...");let l=document.getElementById("ah-diag-console");l&&(l.style.display="block");let g={id:"feature_"+Date.now(),type:"feature",message:r,timestamp:new Date().toISOString(),source:window.location.pathname};this.runDiagnosticEngine(g,o)})}openDiagnosticModal(e,t){if(!this.container)return;t?(document.body.classList.add("ah-blur-active"),this.container.classList.add("ah-hard-crash")):this.container.classList.remove("ah-hard-crash"),this.container.style.display="flex";let s=new Date(e.timestamp).toLocaleTimeString();this.container.innerHTML=`
      <div class="ah-diag-modal">
        <div class="ah-diag-header">
          <div class="ah-diag-title">
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal()),document.getElementById("ah-ignore-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let a=document.getElementById("ah-patch-btn");this.runDiagnosticEngine(e,a)}async runDiagnosticEngine(e,t){let s=document.getElementById("ah-diag-console"),a=document.getElementById("ah-footer-status"),i=document.getElementById("ah-scanner-box"),o=document.getElementById("ah-patch-box"),n=document.getElementById("ah-diff-box");if(!s)return;let r=(l,g="default")=>{let m=document.createElement("div");m.className=`ah-console-line ${g}`,m.textContent=`> ${l}`,s.appendChild(m),s.scrollTop=s.scrollHeight};await this.delay(1e3),r("Analyzing stack trace patterns...","info"),a&&(a.textContent="Analyzing source-code stack trace..."),await this.delay(1200),r("Extracting code context around error location...","info"),await this.delay(1e3),r("Consulting Gemini LLM error-healing patterns...","comment"),a&&(a.textContent="Generating surgical repair code...");let c=!1,h="";if(this.onHealHandler)try{let l=await this.onHealHandler(e);c=l.success,h=l.diffCode}catch{r("Failed to contact Gemini Healer Agent. Checking offline templates.","error")}if(await this.delay(800),c&&h){if(r("Surgical fix generated successfully!","success"),r("Ready to hot-patch runtime environment.","success"),a&&(a.textContent="Patch compiled successfully!"),i&&(i.style.display="none"),o&&(o.style.display="block"),n&&(n.innerHTML=this.renderDiff(h)),t){t.disabled=!1,t.classList.remove("disabled");let l=document.getElementById("ah-btn-loader");l&&(l.style.display="none");let g=document.getElementById("ah-btn-text");g&&(g.textContent="APPLY LIVE PATCH \u{1FA7A}"),t.onclick=async()=>{t.disabled=!0,t.classList.add("disabled"),l&&(l.style.display="inline-block"),g&&(g.textContent="Applying Patch..."),a&&(a.textContent="Injecting runtime hot-patch..."),await this.delay(1200),this.currentErrors=this.currentErrors.filter(m=>m.id!==e.id),this.updateBadgeCount(),this.showToast("\u{1F680} System Healed Successfully!","success"),this.closeDiagnosticModal(),r("Hot-patch successfully applied! Site recovered.","success")}}}else{r("AI agent could not determine a safe patch for this exception.","error"),a&&(a.textContent="Healing failed. Manual debug required.");let l=document.getElementById("ah-btn-loader");l&&(l.style.display="none");let g=document.getElementById("ah-btn-text");g&&(g.textContent="Unable to heal")}}renderDiff(e){return e.split(`
...
+ }`,s="/* Injected CTA conversion elements */",a.conversion=97),this.scores=a,await this.delay(1e3),this.addTerminalLog("Visual upgrade stylesheet compiled successfully!","success"),(await this.dispatchWebhook({file:"playground/src/index.css",content:s,diffCode:t,explanation:`Evolved target design hook: ${e}`,type:"css"})).success){this.addTerminalLog("Evolved scoring variables completed! Redeployment is active.","success");try{let c=await(await fetch("http://localhost:3001/api/scores",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({scores:a})})).json();c.success&&c.scores&&(this.scores=c.scores)}catch(r){this.addTerminalLog(`Failed to write evolved scores to backend DB: ${r.message}`,"error")}let o=`autoheal-evolution-style-${e}`,n=document.getElementById(o);n||(n=document.createElement("style"),n.id=o,document.head.appendChild(n)),e==="animation"?n.textContent=`
          ...
          <div class="ah-dash-column col-left">
            ...
            <div class="ah-panel glass-panel">
              ...
              <div class="ah-panel-body dark-terminal scrollable" id="ah-dash-terminal">
                ${this.currentTerminalLogs.map(d=>{let p="";return d.includes("\u2713")||d.includes("Live")?p="success":d.includes("\u2717")||d.includes("FAILED")?p="error":d.includes("\u2139")||d.includes("Initiating")?p="info":d.includes("comment")&&(p="comment"),`<div class="ah-term-line ${p}">${d}</div>`}).join("")}
              </div>
            </div>
    ...
    `,this.container.querySelectorAll(".heal-trigger-btn").forEach(d=>{d.addEventListener("click",p=>{let f=p.currentTarget.dataset.id,v=this.errorList.find(L=>L.id===f);v&&this.triggerSimulatedHeal(v)})}),this.container.querySelectorAll(".evolve-trigger-btn").forEach(d=>{d.addEventListener("click",p=>{let f=p.currentTarget.dataset.gauge;f&&this.triggerSimulatedEvolution(f)})});let r=this.container.querySelector("#ah-dash-reseed-trigger");r&&r.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let p=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(p.success&&p.errors)this.errorList=p.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let c=this.container.querySelector("#ah-dash-empty-reseed");c&&c.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let p=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(p.success&&p.errors)this.errorList=p.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let h=this.container.querySelector("#ah-dash-settings-trigger"),l=this.container.querySelector("#ah-settings-panel"),g=this.container.querySelector("#ah-settings-cancel"),m=this.container.querySelector("#ah-dash-settings-form");h&&l&&h.addEventListener("click",()=>{l.style.display=l.style.display==="none"?"block":"none"}),g&&l&&g.addEventListener("click",()=>{l.style.display="none"}),m&&l&&m.addEventListener("submit",d=>{d.preventDefault();let p=document.getElementById("ah-n8n-url-input").value,f=document.getElementById("ah-vercel-url-input").value,v=document.getElementById("ah-git-branch-input").value,L=document.getElementById("ah-model-provider-input").value,$=document.getElementById("ah-gemini-key-input").value,A=document.getElementById("ah-groq-key-input").value;this.saveSettings(p,f,v,L,$,A),l.style.display="none"});let k=document.getElementById("ah-dash-terminal");k&&(k.scrollTop=k.scrollHeight)}delay(e){return new Promise(t=>setTimeout(t,e))}injectStyles(){if(document.getElementById("autoheal-dashboard-styles"))return;let e=document.createElement("style");e.id="autoheal-dashboard-styles",e.textContent=`
      @import url('https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;700&family=Inter:wght@400;500;600;700&display=swap');
