...
===================================================
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal()),document.getElementById("ah-ignore-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let a=document.getElementById("ah-patch-btn");this.runDiagnosticEngine(e,a)}async runDiagnosticEngine(e,t){let s=document.getElementById("ah-diag-console"),a=document.getElementById("ah-footer-status"),i=document.getElementById("ah-scanner-box"),o=document.getElementById("ah-patch-box"),n=document.getElementById("ah-diff-box");if(!s)return;let r=(d,m="default")=>{let f=document.createElement("div");f.className=`ah-console-line ${m}`,f.textContent=`> ${d}`,s.appendChild(f),s.scrollTop=s.scrollHeight};await this.delay(1e3),r("Analyzing stack trace patterns...","info"),a&&(a.textContent="Analyzing source-code stack trace..."),await this.delay(1200),r("Extracting code context around error location...","info"),await this.delay(1e3),r("Consulting AI LLM healing patterns...","comment"),a&&(a.textContent="Generating surgical repair code...");let c=!1,g="",p="";if(this.onHealHandler)try{let d=await this.onHealHandler(e);c=d.success,g=d.diffCode,p=d.explanation||""}catch{r("Failed to contact AI Healer Agent.","error")}if(await this.delay(800),c&&g){if(r("Surgical fix generated successfully!","success"),r("Ready to hot-patch runtime environment.","success"),a&&(a.textContent="Patch compiled successfully!"),i&&(i.style.display="none"),o&&(o.style.display="block"),n&&(n.innerHTML=this.renderDiff(g)),t){t.disabled=!1,t.classList.remove("disabled");let d=document.getElementById("ah-btn-loader");d&&(d.style.display="none");let m=document.getElementById("ah-btn-text");m&&(m.textContent="APPLY LIVE PATCH \u{1FA7A}"),t.onclick=async()=>{t.disabled=!0,t.classList.add("disabled"),d&&(d.style.display="inline-block"),m&&(m.textContent="Applying Patch..."),a&&(a.textContent="Injecting runtime hot-patch..."),await this.delay(1200),this.currentErrors=this.currentErrors.filter(f=>f.id!==e.id),this.updateBadgeCount(),this.showToast("\u{1F680} System Healed Successfully!","success"),this.closeDiagnosticModal(),r("Hot-patch successfully applied! Site recovered.","success")}}}else if(r(`AI agent failed: ${p||"Could not determine a safe patch."}`,"error"),a&&(a.textContent="Healing failed. Manual debug required."),t){t.disabled=!1,t.classList.remove("disabled");let d=document.getElementById("ah-btn-loader");d&&(d.style.display="none");let m=document.getElementById("ah-btn-text");m&&(m.textContent="Unable to heal")}}renderDiff(e){return e.split(`
...
+ }`,s="/* Injected CTA conversion elements */",a.conversion=97),this.scores=a,await this.delay(1e3),this.addTerminalLog("Visual upgrade stylesheet compiled successfully!","success"),(await this.dispatchWebhook({file:"playground/src/index.css",content:s,diffCode:t,explanation:`Evolved target design hook: ${e}`,type:"css"})).success){this.addTerminalLog("Evolved scoring variables completed! Redeployment is active.","success");try{let c=await(await fetch("http://localhost:3001/api/scores",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({scores:a})})).json();c.success&&c.scores&&(this.scores=c.scores)}catch(r){this.addTerminalLog(`Failed to write evolved scores to backend DB: ${r.message}`,"error")}let o=`autoheal-evolution-style-${e}`,n=document.getElementById(o);n||(n=document.createElement("style"),n.id=o,document.head.appendChild(n)),e==="animation"?n.textContent=`
          ...
          <div class="ah-dash-column col-left">
            ...
            <div class="ah-panel glass-panel">
              ...
              <div class="ah-panel-body dark-terminal scrollable" id="ah-dash-terminal">
                ${this.currentTerminalLogs.map(l=>{let h="";return l.includes("\u2713")||l.includes("Live")?h="success":l.includes("\u2717")||l.includes("FAILED")?h="error":l.includes("\u2139")||l.includes("Initiating")?h="info":l.includes("comment")&&(h="comment"),`<div class="ah-term-line ${h}">${l}</div>`}).join("")}
              </div>
            </div>
    ...
    `,this.container.querySelectorAll(".heal-trigger-btn").forEach(l=>{l.addEventListener("click",h=>{let b=h.currentTarget.dataset.id,x=this.errorList.find(L=>L.id===b);x&&this.triggerSimulatedHeal(x)})}),this.container.querySelectorAll(".evolve-trigger-btn").forEach(l=>{l.addEventListener("click",h=>{let b=h.currentTarget.dataset.gauge;b&&this.triggerSimulatedEvolution(b)})});let r=this.container.querySelector("#ah-dash-reseed-trigger");r&&r.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let h=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(h.success&&h.errors)this.errorList=h.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let c=this.container.querySelector("#ah-dash-empty-reseed");c&&c.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let h=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(h.success&&h.errors)this.errorList=h.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let g=this.container.querySelector("#ah-dash-settings-trigger"),p=this.container.querySelector("#ah-settings-panel"),d=this.container.querySelector("#ah-settings-cancel"),m=this.container.querySelector("#ah-dash-settings-form");g&&p&&g.addEventListener("click",()=>{p.style.display=p.style.display==="none"?"block":"none"}),d&&p&&d.addEventListener("click",()=>{p.style.display="none"}),m&&p&&m.addEventListener("submit",l=>{l.preventDefault();let h=document.getElementById("ah-n8n-url-input").value,b=document.getElementById("ah-vercel-url-input").value,x=document.getElementById("ah-git-branch-input").value,L=document.getElementById("ah-model-provider-input").value,$=document.getElementById("ah-gemini-key-input").value,A=document.getElementById("ah-groq-key-input").value;this.saveSettings(h,b,x,L,$,A),p.style.display="none"});let f=document.getElementById("ah-dash-terminal");f&&(f.scrollTop=f.scrollHeight)}delay(e){return new Promise(t=>setTimeout(t,e))}injectStyles(){if(document.getElementById("autoheal-dashboard-styles"))return;let e=document.createElement("style");e.id="autoheal-dashboard-styles",e.textContent=`
      @import url('https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;700&family=Inter:wght@400;500;600;700&display=swap');
