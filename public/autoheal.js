...
===================================================
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let e=document.getElementById("ah-build-btn"),t=document.getElementById("ah-feature-prompt");e.addEventListener("click",()=>{let s=t.value.trim();if(!s)return;t.disabled=!0,e.disabled=!0,e.classList.add("disabled");let a=document.getElementById("ah-btn-loader");a&&(a.style.display="inline-block");let i=document.getElementById("ah-btn-text");i&&(i.textContent="BUILDING...");let n=document.getElementById("ah-diag-console");n&&(n.style.display="block");let o={id:"feature_"+Date.now(),type:"feature",message:s,timestamp:new Date().toISOString(),source:window.location.pathname};this.runDiagnosticEngine(o,e)})}openDiagnosticModal(e,t){if(!this.container)return;t?(document.body.classList.add("ah-blur-active"),this.container.classList.add("ah-hard-crash")):this.container.classList.remove("ah-hard-crash"),this.container.style.display="flex";let s=new Date(e.timestamp).toLocaleTimeString();this.container.innerHTML=`
      <div class="ah-diag-modal">
        <div class="ah-diag-header">
          <div class="ah-diag-title">
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal()),document.getElementById("ah-ignore-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let a=document.getElementById("ah-patch-btn");this.runDiagnosticEngine(e,a)}async runDiagnosticEngine(e,t){let s=document.getElementById("ah-diag-console"),a=document.getElementById("ah-footer-status"),i=document.getElementById("ah-scanner-box"),n=document.getElementById("ah-patch-box"),o=document.getElementById("ah-diff-box");if(!s)return;let r=(d,u="default")=>{let m=document.createElement("div");m.className=`ah-console-line ${u}`,m.textContent=`> ${d}`,s.appendChild(m),s.scrollTop=s.scrollHeight};await this.delay(1e3),r("Analyzing stack trace patterns...","info"),a&&(a.textContent="Analyzing source-code stack trace..."),await this.delay(1200),r("Extracting code context around error location...","info"),await this.delay(1e3),r("Consulting Gemini LLM error-healing patterns...","comment"),a&&(a.textContent="Generating surgical repair code...");let p=!1,g="";if(this.onHealHandler)try{let d=await this.onHealHandler(e);p=d.success,g=d.diffCode}catch{r("Failed to contact Gemini Healer Agent. Checking offline templates.","error")}if(await this.delay(800),p&&g){if(r("Surgical fix generated successfully!","success"),r("Ready to hot-patch runtime environment.","success"),a&&(a.textContent="Patch compiled successfully!"),i&&(i.style.display="none"),n&&(n.style.display="block"),o&&(o.innerHTML=this.renderDiff(g)),t){t.disabled=!1,t.classList.remove("disabled");let d=document.getElementById("ah-btn-loader");d&&(d.style.display="none");let u=document.getElementById("ah-btn-text");u&&(u.textContent="APPLY LIVE PATCH \u{1FA7A}"),t.onclick=async()=>{t.disabled=!0,t.classList.add("disabled"),d&&(d.style.display="inline-block"),u&&(u.textContent="Applying Patch..."),a&&(a.textContent="Injecting runtime hot-patch..."),await this.delay(1200),this.currentErrors=this.currentErrors.filter(m=>m.id!==e.id),this.updateBadgeCount(),this.showToast("\u{1F680} System Healed Successfully!","success"),this.closeDiagnosticModal(),r("Hot-patch successfully applied! Site recovered.","success")}}}else{r("AI agent could not determine a safe patch for this exception.","error"),a&&(a.textContent="Healing failed. Manual debug required.");let d=document.getElementById("ah-btn-loader");d&&(d.style.display="none");let u=document.getElementById("ah-btn-text");u&&(u.textContent="Unable to heal")}}renderDiff(e){return e.split(`
...
+ }`,s="/* Injected CTA conversion elements */",a.conversion=97),this.scores=a,await this.delay(1e3),this.addTerminalLog("Visual upgrade stylesheet compiled successfully!","success"),(await this.dispatchWebhook({file:"playground/src/index.css",content:s,diffCode:t,explanation:`Evolved target design hook: ${e}`,type:"css"})).success){this.addTerminalLog("Evolved scoring variables completed! Redeployment is active.","success");try{let p=await(await fetch("http://localhost:3001/api/scores",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({scores:a})})).json();p.success&&p.scores&&(this.scores=p.scores)}catch(r){this.addTerminalLog(`Failed to write evolved scores to backend DB: ${r.message}`,"error")}let n=`autoheal-evolution-style-${e}`,o=document.getElementById(n);o||(o=document.createElement("style"),o.id=n,document.head.appendChild(o)),e==="animation"?o.textContent=`
        ...
        `),this.render()}}render(){if(!this.container)return;let e=this.settings.n8nWebhook||"",t=this.settings.vercelDeployHook||"",s=this.settings.gitBranch||"main",a=this.scores.polish,i=this.scores.spacing,n=this.scores.mobile,o=this.scores.conversion;this.container.innerHTML=`
          ...
          <div class="ah-dash-column col-left">
            ...
            <div class="ah-panel glass-panel">
              ...
              <div class="ah-panel-body dark-terminal scrollable" id="ah-dash-terminal">
                ${this.currentTerminalLogs.map(l=>{let c="";return l.includes("\u2713")||l.includes("Live")?c="success":l.includes("\u2717")||l.includes("FAILED")?c="error":l.includes("\u2139")||l.includes("Initiating")?c="info":l.includes("comment")&&(c="comment"),`<div class="ah-term-line ${c}">${l}</div>`}).join("")}
              </div>
            </div>
              ...
              <div class="ah-panel-body">
                <div class="ah-gauges-container">
                  ...
                  <div class="ah-gauge-row">
                    ...
                    <div class="ah-progress-track">
                      <div class="ah-progress-fill emerald" style="width: ${o}%;"></div>
                    </div>
                    <div class="ah-gauge-footer">
                      <span class="ah-gauge-desc">${o>=80?"Glowing CTA gradient borders loaded":"Low CTA layout visibility"}</span>
    ...
    `,this.container.querySelectorAll(".heal-trigger-btn").forEach(l=>{l.addEventListener("click",c=>{let f=c.currentTarget.dataset.id,x=this.errorList.find(L=>L.id===f);x&&this.triggerSimulatedHeal(x)})}),this.container.querySelectorAll(".evolve-trigger-btn").forEach(l=>{l.addEventListener("click",c=>{let f=c.currentTarget.dataset.gauge;f&&this.triggerSimulatedEvolution(f)})});let r=this.container.querySelector("#ah-dash-reseed-trigger");r&&r.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let c=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(c.success&&c.errors)this.errorList=c.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let p=this.container.querySelector("#ah-dash-empty-reseed");p&&p.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let c=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(c.success&&c.errors)this.errorList=c.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let g=this.container.querySelector("#ah-dash-settings-trigger"),d=this.container.querySelector("#ah-settings-panel"),u=this.container.querySelector("#ah-settings-cancel"),m=this.container.querySelector("#ah-dash-settings-form");g&&d&&g.addEventListener("click",()=>{d.style.display=d.style.display==="none"?"block":"none"}),u&&d&&u.addEventListener("click",()=>{d.style.display="none"}),m&&d&&m.addEventListener("submit",l=>{l.preventDefault();let c=document.getElementById("ah-n8n-url-input").value,f=document.getElementById("ah-vercel-url-input").value,x=document.getElementById("ah-git-branch-input").value,L=document.getElementById("ah-model-provider-input").value,H=document.getElementById("ah-gemini-key-input").value,A=document.getElementById("ah-groq-key-input").value;this.saveSettings(c,f,x,L,H,A),d.style.display="none"});let k=document.getElementById("ah-dash-terminal");k&&(k.scrollTop=k.scrollHeight)}delay(e){return new Promise(t=>setTimeout(t,e))}injectStyles(){if(document.getElementById("autoheal-dashboard-styles"))return;let e=document.createElement("style");e.id="autoheal-dashboard-styles",e.textContent=`
      @import url('https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;700&family=Inter:wght@400;500;600;700&display=swap');
    ...
    `,document.head.appendChild(e)}},w=new D;var E=class{interceptor=new v;config={autoHealEnabled:!0};initialized=!1;caughtErrors=[];constructor(){}init(e){this.initialized||(this.initialized=!0,this.config={...this.config,...e},e.email&&y.setConfig(e.email),window.__autoheal_errors_cache__=this.caughtErrors,b.init(async t=>{if(this.config.onHealRequest)return await this.config.onHealRequest(t);let s=t.type==="feature",a=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",i=window.AUTOHEAL_SITE_ID||window.location.host;try{let o=await(await fetch(`${a}/api/generate-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":i},body:JSON.stringify(s?{prompt:t.message,file:t.source||"sandbox"}:{error:t,file:t.source||"sandbox"})})).json();if(!o.success)throw new Error(o.explanation);return await fetch(`${a}/api/apply-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":i},body:JSON.stringify({file:o.targetPath||"sandbox",content:o.healedFileContent})}),{success:!0,diffCode:o.diffCode}}catch(n){return console.error("__autoheal_internal__ Standalone generation error:",n),{success:!1,diffCode:"",explanation:n.message}}}),this.interceptor.start(t=>{this.caughtErrors.push(t),window.__autoheal_errors_cache__=this.caughtErrors,fetch("http://localhost:3001/api/telemetry",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({error:t})}).then(s=>s.json()).then(s=>{s.success&&s.errors&&window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:s.errors}))}).catch(s=>{console.warn("__autoheal_internal__ Telemetry database sync warning:",s),window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.caughtErrors}))}),y.sendErrorEmail(t),t.type==="crash"?b.triggerHardCrashOverlay(t):b.reportSoftError(t)}),console.log("__autoheal_internal__ AutoHealUI SDK active and monitoring logs."))}mountDashboard(e){w.mount(e)}shutdown(){this.interceptor.stop(),this.initialized=!1}get getConfig(){return this.config}get patcher(){return C}get emailer(){return y}get widget(){return b}get dashboard(){return w}},$=new E;typeof window<"u"&&window.AUTOHEAL_SITE_ID&&setTimeout(()=>{$.init({})},100);return N(R);})();
