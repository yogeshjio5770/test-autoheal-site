...
===================================================
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal()),document.getElementById("ah-ignore-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let a=document.getElementById("ah-patch-btn");this.runDiagnosticEngine(e,a)}async runDiagnosticEngine(e,t){let s=document.getElementById("ah-diag-console"),a=document.getElementById("ah-footer-status"),i=document.getElementById("ah-scanner-box"),n=document.getElementById("ah-patch-box"),o=document.getElementById("ah-diff-box");if(!s)return;let r=(l,d="default")=>{let g=document.createElement("div");g.className=`ah-console-line ${d}`,g.textContent=`> ${l}`,s.appendChild(g),s.scrollTop=s.scrollHeight};await this.delay(1e3),r("Analyzing stack trace patterns...","info"),a&&(a.textContent="Analyzing source-code stack trace..."),await this.delay(1200),r("Extracting code context around error location...","info"),await this.delay(1e3),r("Consulting AI LLM healing patterns...","comment"),a&&(a.textContent="Generating surgical repair code...");let p=!1,h="",c="",m="",b=e.source||"sandbox",f=[];if(this.onHealHandler)try{let l=await this.onHealHandler(e);p=l.success,h=l.diffCode,c=l.explanation||"",m=l.healedFileContent||"",l.targetPath&&(b=l.targetPath),l.files&&(f=l.files)}catch{r("Failed to contact AI Healer Agent.","error")}if(await this.delay(800),p&&h){if(r("Surgical fix generated successfully!","success"),r("Ready to hot-patch runtime environment.","success"),a&&(a.textContent="Patch compiled successfully!"),i&&(i.style.display="none"),n&&(n.style.display="block"),o&&(o.innerHTML=this.renderDiff(h)),t){t.disabled=!1,t.classList.remove("disabled");let l=document.getElementById("ah-btn-loader");l&&(l.style.display="none");let d=document.getElementById("ah-btn-text");d&&(d.textContent="APPLY LIVE PATCH \u{1FA7A}"),t.onclick=async()=>{t.disabled=!0,t.classList.add("disabled"),l&&(l.style.display="inline-block"),d&&(d.textContent="Applying Patch..."),a&&(a.textContent="Injecting runtime hot-patch...");try{let g=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",y=window.AUTOHEAL_SITE_ID||window.location.host,E=await(await fetch(`${g}/api/apply-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":y},body:JSON.stringify({files:f.length>0?f:void 0,content:m,file:b,prompt:e.message})})).json();E.success?this.showToast("\u{1F680} Code Pushed to GitHub! Vercel is building...","success"):this.showToast(`\u274C Push Failed: ${E.error}`,"error")}catch{this.showToast("\u274C Network error communicating with Master Server","error")}this.currentErrors=this.currentErrors.filter(g=>g.id!==e.id),this.updateBadgeCount(),this.closeDiagnosticModal(),r("Hot-patch successfully applied! Site recovered.","success")}}}else if(r(`AI agent failed: ${c||"Could not determine a safe patch."}`,"error"),a&&(a.textContent="Healing failed. Manual debug required."),t){t.disabled=!1,t.classList.remove("disabled");let l=document.getElementById("ah-btn-loader");l&&(l.style.display="none");let d=document.getElementById("ah-btn-text");d&&(d.textContent="Unable to heal")}}renderDiff(e){return e.split(`
...
@@ -1,1 +1,2 @@
- /* error */
+ /* simulated AI cloud repair applied successfully */`;if(i&&n){if(this.addTerminalLog("AI Repair patch compiled successfully!","success"),this.addTerminalLog("Printing visual git patch diff...","info"),await this.printDiffToTerminal(n),(await this.dispatchWebhook({file:o,content:"/* Completed autonomous code replacement script context */",diffCode:n,explanation:`Surgically repaired exception: ${e.message}`,type:e.type})).success){try{let h=await(await fetch("http://localhost:3001/api/telemetry/clear",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({id:e.id})})).json();h.success&&h.errors?(this.errorList=h.errors,window.__autoheal_errors_cache__=this.errorList):(this.errorList=this.errorList.filter(c=>c.id!==e.id),window.__autoheal_errors_cache__=this.errorList)}catch{this.errorList=this.errorList.filter(h=>h.id!==e.id),window.__autoheal_errors_cache__=this.errorList}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()}}else this.addTerminalLog("Repair algorithm aborted: AI agent could not generate safe replacement boundaries.","error")}async triggerSimulatedEvolution(e){this.addTerminalLog(`Starting visual layout evolution: [${e.toUpperCase()}]`,"info"),this.addTerminalLog("Studying DOM node alignment and mobile styling parameters...","comment"),await this.delay(1200),this.addTerminalLog("Generating updated Glassmorphic token stylesheet...","info");let t="",s="",a={...this.scores};if(e==="animation"?(t=`+ .sandbox-card {
...
+ }`,s="/* Injected CTA conversion elements */",a.conversion=97),this.scores=a,await this.delay(1e3),this.addTerminalLog("Visual upgrade stylesheet compiled successfully!","success"),(await this.dispatchWebhook({file:"playground/src/index.css",content:s,diffCode:t,explanation:`Evolved target design hook: ${e}`,type:"css"})).success){this.addTerminalLog("Evolved scoring variables completed! Redeployment is active.","success");try{let p=await(await fetch("http://localhost:3001/api/scores",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({scores:a})})).json();p.success&&p.scores&&(this.scores=p.scores)}catch(r){this.addTerminalLog(`Failed to write evolved scores to backend DB: ${r.message}`,"error")}let n=`autoheal-evolution-style-${e}`,o=document.getElementById(n);o||(o=document.createElement("style"),o.id=n,document.head.appendChild(o)),e==="animation"?o.textContent=`
    ...
    `,this.container.querySelectorAll(".heal-trigger-btn").forEach(l=>{l.addEventListener("click",d=>{let g=d.currentTarget.dataset.id,y=this.errorList.find(w=>w.id===g);y&&this.triggerSimulatedHeal(y)})}),this.container.querySelectorAll(".evolve-trigger-btn").forEach(l=>{l.addEventListener("click",d=>{let g=d.currentTarget.dataset.gauge;g&&this.triggerSimulatedEvolution(g)})});let r=this.container.querySelector("#ah-dash-reseed-trigger");r&&r.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let d=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(d.success&&d.errors)this.errorList=d.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let p=this.container.querySelector("#ah-dash-empty-reseed");p&&p.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let d=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(d.success&&d.errors)this.errorList=d.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let h=this.container.querySelector("#ah-dash-settings-trigger"),c=this.container.querySelector("#ah-settings-panel"),m=this.container.querySelector("#ah-settings-cancel"),b=this.container.querySelector("#ah-dash-settings-form");h&&c&&h.addEventListener("click",()=>{c.style.display=c.style.display==="none"?"block":"none"}),m&&c&&m.addEventListener("click",()=>{c.style.display="none"}),b&&c&&b.addEventListener("submit",l=>{l.preventDefault();let d=document.getElementById("ah-n8n-url-input").value,g=document.getElementById("ah-vercel-url-input").value,y=document.getElementById("ah-git-branch-input").value,w=document.getElementById("ah-model-provider-input").value,E=document.getElementById("ah-gemini-key-input").value,$=document.getElementById("ah-groq-key-input").value;this.saveSettings(d,g,y,w,E,$),c.style.display="none"});let f=document.getElementById("ah-dash-terminal");f&&(f.scrollTop=f.scrollHeight)}delay(e){return new Promise(t=>setTimeout(t,e))}injectStyles(){if(document.getElementById("autoheal-dashboard-styles"))return;let e=document.createElement("style");e.id="autoheal-dashboard-styles",e.textContent=`
      @import url('https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;700&family=Inter:wght@400;500;600;700&display=swap');
    ...
    `,document.head.appendChild(e)}},L=new A;var S=class{interceptor=new k;config={autoHealEnabled:!0};initialized=!1;caughtErrors=[];constructor(){}init(e){this.initialized||(this.initialized=!0,this.config={...this.config,...e},e.email&&x.setConfig(e.email),window.__autoheal_errors_cache__=this.caughtErrors,v.init(async t=>{if(this.config.onHealRequest)return await this.config.onHealRequest(t);let s=t.type==="feature",a=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",i=window.AUTOHEAL_SITE_ID||window.location.host;try{let o=await(await fetch(`${a}/api/generate-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":i},body:JSON.stringify(s?{prompt:t.message,file:t.source||"sandbox"}:{error:t,file:t.source||"sandbox"})})).json();if(!o.success)throw new Error(o.explanation);return{success:!0,diffCode:o.diffCode,files:o.files||[],healedFileContent:o.healedFileContent,targetPath:o.targetPath||"sandbox"}}catch(n){return console.error("__autoheal_internal__ Standalone generation error:",n),{success:!1,diffCode:"",explanation:n.message}}}),this.interceptor.start(t=>{this.caughtErrors.push(t),window.__autoheal_errors_cache__=this.caughtErrors,fetch("http://localhost:3001/api/telemetry",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({error:t})}).then(s=>s.json()).then(s=>{s.success&&s.errors&&window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:s.errors}))}).catch(s=>{console.warn("__autoheal_internal__ Telemetry database sync warning:",s),window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.caughtErrors}))}),x.sendErrorEmail(t),t.type==="crash"?v.triggerHardCrashOverlay(t):v.reportSoftError(t)}),console.log("__autoheal_internal__ AutoHealUI SDK active and monitoring logs."))}mountDashboard(e){L.mount(e)}shutdown(){this.interceptor.stop(),this.initialized=!1}get getConfig(){return this.config}get patcher(){return C}get emailer(){return x}get widget(){return v}get dashboard(){return L}},H=new S;typeof window<"u"&&window.AUTOHEAL_SITE_ID&&setTimeout(()=>{H.init({})},100);return j(R);})();
