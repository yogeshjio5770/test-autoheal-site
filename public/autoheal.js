...
===================================================
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let e=document.getElementById("ah-feature-view"),t=document.getElementById("ah-feature-footer"),s=document.getElementById("ah-settings-view"),a=document.getElementById("ah-settings-footer"),i=!1;document.getElementById("ah-settings-btn")?.addEventListener("click",async()=>{if(i=!i,i){e.style.display="none",t.style.display="none",s.style.display="block",a.style.display="flex";let r=document.getElementById("ah-autonomous-toggle");r&&(r.checked=localStorage.getItem("autoheal_autonomous")==="true");try{let c=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",p=window.AUTOHEAL_SITE_ID||window.location.host,g=await(await fetch(`${c}/api/settings`,{headers:{"x-site-id":p}})).json();g.groqKey&&(document.getElementById("ah-groq-key-input").value=g.groqKey)}catch(c){console.warn("AutoHeal: Could not fetch settings",c)}}else e.style.display="block",t.style.display="flex",s.style.display="none",a.style.display="none"}),document.getElementById("ah-save-settings-btn")?.addEventListener("click",async()=>{let r=document.getElementById("ah-groq-key-input").value.trim(),c=document.getElementById("ah-autonomous-toggle");if(c&&localStorage.setItem("autoheal_autonomous",c.checked?"true":"false"),!r){let f=document.getElementById("ah-settings-status");f.textContent="Settings saved locally.",setTimeout(()=>{f.textContent=""},3e3);return}let p=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",h=window.AUTOHEAL_SITE_ID||window.location.host,g=document.getElementById("ah-settings-status");g.textContent="Saving...";try{(await(await fetch(`${p}/api/settings`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":h},body:JSON.stringify({settings:{groqKey:r,modelProvider:"groq"}})})).json()).success?g.textContent="Settings saved successfully! \u2705":g.textContent="Error saving settings."}catch{g.textContent="Failed to connect to Master Server."}});let o=document.getElementById("ah-build-btn"),n=document.getElementById("ah-feature-prompt");o.addEventListener("click",()=>{let r=n.value.trim();if(!r)return;n.disabled=!0,o.disabled=!0,o.classList.add("disabled");let c=document.getElementById("ah-btn-loader");c&&(c.style.display="inline-block");let p=document.getElementById("ah-btn-text");p&&(p.textContent="BUILDING...");let h=document.getElementById("ah-diag-console");h&&(h.style.display="block");let g={id:"feature_"+Date.now(),type:"feature",message:r,timestamp:new Date().toISOString(),source:window.location.pathname==="/"?"src/App.jsx":window.location.pathname};this.runDiagnosticEngine(g,o)})}openDiagnosticModal(e,t){if(!this.container)return;t?(document.body.classList.add("ah-blur-active"),this.container.classList.add("ah-hard-crash")):this.container.classList.remove("ah-hard-crash"),this.container.style.display="flex";let s=new Date(e.timestamp).toLocaleTimeString();this.container.innerHTML=`
      <div class="ah-diag-modal">
        <div class="ah-diag-header">
          <div class="ah-diag-title">
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal()),document.getElementById("ah-ignore-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let a=document.getElementById("ah-patch-btn");this.runDiagnosticEngine(e,a)}async runDiagnosticEngine(e,t){let s=document.getElementById("ah-diag-console"),a=document.getElementById("ah-footer-status"),i=document.getElementById("ah-scanner-box"),o=document.getElementById("ah-patch-box"),n=document.getElementById("ah-diff-box");if(!s)return;let r=(l,d="default")=>{let u=document.createElement("div");u.className=`ah-console-line ${d}`,u.textContent=`> ${l}`,s.appendChild(u),s.scrollTop=s.scrollHeight};await this.delay(1e3),r("Analyzing stack trace patterns...","info"),a&&(a.textContent="Analyzing source-code stack trace..."),await this.delay(1200),r("Extracting code context around error location...","info"),await this.delay(1e3),r("Consulting AI LLM healing patterns...","comment"),a&&(a.textContent="Generating surgical repair code...");let c=!1,p="",h="",g="",f=e.source||"sandbox",b=[];if(this.onHealHandler)try{let l=await this.onHealHandler(e);c=l.success,p=l.diffCode,h=l.explanation||"",g=l.healedFileContent||"",l.targetPath&&(f=l.targetPath),l.files&&(b=l.files)}catch{r("Failed to contact AI Healer Agent.","error")}if(await this.delay(800),c&&p){if(r("Surgical fix generated successfully!","success"),r("Ready to hot-patch runtime environment.","success"),a&&(a.textContent="Patch compiled successfully!"),i&&(i.style.display="none"),o&&(o.style.display="block"),n&&(n.innerHTML=this.renderDiff(p)),t){t.disabled=!1,t.classList.remove("disabled");let l=document.getElementById("ah-btn-loader");l&&(l.style.display="none");let d=document.getElementById("ah-btn-text");d&&(d.textContent="APPLY LIVE PATCH \u{1FA7A}"),t.onclick=async()=>{t.disabled=!0,t.classList.add("disabled"),l&&(l.style.display="inline-block"),d&&(d.textContent="Applying Patch..."),a&&(a.textContent="Injecting runtime hot-patch...");try{let u=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",x=window.AUTOHEAL_SITE_ID||window.location.host,E=await(await fetch(`${u}/api/apply-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":x},body:JSON.stringify({files:b.length>0?b:void 0,content:g,file:f,prompt:e.message})})).json();E.success?this.showToast("\u{1F680} Code Pushed to GitHub! Vercel is building...","success"):this.showToast(`\u274C Push Failed: ${E.error}`,"error")}catch{this.showToast("\u274C Network error communicating with Master Server","error")}this.currentErrors=this.currentErrors.filter(u=>u.id!==e.id),this.updateBadgeCount(),this.closeDiagnosticModal(),r("Hot-patch successfully applied! Site recovered.","success")}}}else if(r(`AI agent failed: ${h||"Could not determine a safe patch."}`,"error"),a&&(a.textContent="Healing failed. Manual debug required."),t){t.disabled=!1,t.classList.remove("disabled");let l=document.getElementById("ah-btn-loader");l&&(l.style.display="none");let d=document.getElementById("ah-btn-text");d&&(d.textContent="Unable to heal")}}renderDiff(e){return e.split(`
...
+ }`,s="/* Injected CTA conversion elements */",a.conversion=97),this.scores=a,await this.delay(1e3),this.addTerminalLog("Visual upgrade stylesheet compiled successfully!","success"),(await this.dispatchWebhook({file:"playground/src/index.css",content:s,diffCode:t,explanation:`Evolved target design hook: ${e}`,type:"css"})).success){this.addTerminalLog("Evolved scoring variables completed! Redeployment is active.","success");try{let c=await(await fetch("http://localhost:3001/api/scores",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({scores:a})})).json();c.success&&c.scores&&(this.scores=c.scores)}catch(r){this.addTerminalLog(`Failed to write evolved scores to backend DB: ${r.message}`,"error")}let o=`autoheal-evolution-style-${e}`,n=document.getElementById(o);n||(n=document.createElement("style"),n.id=o,document.head.appendChild(n)),e==="animation"?n.textContent=`
    ...
    `,this.container.querySelectorAll(".heal-trigger-btn").forEach(l=>{l.addEventListener("click",d=>{let u=d.currentTarget.dataset.id,x=this.errorList.find(w=>w.id===u);x&&this.triggerSimulatedHeal(x)})}),this.container.querySelectorAll(".evolve-trigger-btn").forEach(l=>{l.addEventListener("click",d=>{let u=d.currentTarget.dataset.gauge;u&&this.triggerSimulatedEvolution(u)})});let r=this.container.querySelector("#ah-dash-reseed-trigger");r&&r.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let d=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(d.success&&d.errors)this.errorList=d.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Reset & populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let c=this.container.querySelector("#ah-dash-empty-reseed");c&&c.addEventListener("click",async()=>{try{this.addTerminalLog("Requesting live mock database re-seeding...","comment");let d=await(await fetch("http://localhost:3001/api/telemetry/reseed",{method:"POST",headers:{"x-site-id":window.location.host}})).json();if(d.success&&d.errors)this.errorList=d.errors,window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated live mock telemetry database successfully.","success");else throw new Error("Reseed request failed on server.")}catch{this.errorList=this.getMockErrors(),window.__autoheal_errors_cache__=this.errorList,this.addTerminalLog("Populated local mock telemetry dataset.","info")}window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.errorList})),this.render()});let p=this.container.querySelector("#ah-dash-settings-trigger"),h=this.container.querySelector("#ah-settings-panel"),g=this.container.querySelector("#ah-settings-cancel"),f=this.container.querySelector("#ah-dash-settings-form");p&&h&&p.addEventListener("click",()=>{h.style.display=h.style.display==="none"?"block":"none"}),g&&h&&g.addEventListener("click",()=>{h.style.display="none"}),f&&h&&f.addEventListener("submit",l=>{l.preventDefault();let d=document.getElementById("ah-n8n-url-input").value,u=document.getElementById("ah-vercel-url-input").value,x=document.getElementById("ah-git-branch-input").value,w=document.getElementById("ah-model-provider-input").value,E=document.getElementById("ah-gemini-key-input").value,$=document.getElementById("ah-groq-key-input").value;this.saveSettings(d,u,x,w,E,$),h.style.display="none"});let b=document.getElementById("ah-dash-terminal");b&&(b.scrollTop=b.scrollHeight)}delay(e){return new Promise(t=>setTimeout(t,e))}injectStyles(){if(document.getElementById("autoheal-dashboard-styles"))return;let e=document.createElement("style");e.id="autoheal-dashboard-styles",e.textContent=`
      @import url('https://fonts.googleapis.com/css2?family=Fira+Code:wght@400;500;700&family=Inter:wght@400;500;600;700&display=swap');
    ...
    `,document.head.appendChild(e)}},T=new A;var L=class{interceptor=new k;config={autoHealEnabled:!0};initialized=!1;caughtErrors=[];constructor(){}init(e){this.initialized||(this.initialized=!0,this.config={...this.config,...e},e.email&&v.setConfig(e.email),window.__autoheal_errors_cache__=this.caughtErrors,y.init(async t=>{if(this.config.onHealRequest)return await this.config.onHealRequest(t);let s=t.type==="feature",a=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",i=window.AUTOHEAL_SITE_ID||window.location.host;try{let n=await(await fetch(`${a}/api/generate-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":i},body:JSON.stringify(s?{prompt:t.message,file:t.source||"sandbox"}:{error:t,file:t.source||"sandbox"})})).json();if(!n.success)throw new Error(n.explanation);return{success:!0,diffCode:n.diffCode,files:n.files||[],healedFileContent:n.healedFileContent,targetPath:n.targetPath||"sandbox"}}catch(o){return console.error("__autoheal_internal__ Standalone generation error:",o),{success:!1,diffCode:"",explanation:o.message}}}),this.interceptor.start(t=>{this.caughtErrors.push(t),window.__autoheal_errors_cache__=this.caughtErrors,fetch("http://localhost:3001/api/telemetry",{method:"POST",headers:{"Content-Type":"application/json","x-site-id":window.location.host},body:JSON.stringify({error:t})}).then(a=>a.json()).then(a=>{a.success&&a.errors&&window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:a.errors}))}).catch(a=>{console.warn("__autoheal_internal__ Telemetry database sync warning:",a),window.dispatchEvent(new CustomEvent("__autoheal_telemetry_update__",{detail:this.caughtErrors}))}),v.sendErrorEmail(t),localStorage.getItem("autoheal_autonomous")==="true"&&t.type!=="feature"?(y.showToast("\u26A1 Autonomous Mode: Analyzing crash in background...","info"),this.runAutonomousHealer(t,y)):t.type==="crash"?y.triggerHardCrashOverlay(t):y.reportSoftError(t)}),console.log("__autoheal_internal__ AutoHealUI SDK active and monitoring logs."))}async runAutonomousHealer(e,t){try{let s=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",a=window.AUTOHEAL_SITE_ID||window.location.host,o=await(await fetch(`${s}/api/generate-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":a},body:JSON.stringify({error:e,file:e.source||"sandbox"})})).json();if(!o.success)throw new Error(o.explanation);t.showToast("\u{1F52E} Autonomous Mode: Patch generated! Deploying to GitHub...","info");let r=await(await fetch(`${s}/api/apply-patch`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":a},body:JSON.stringify({files:o.files&&o.files.length>0?o.files:void 0,content:o.healedFileContent,file:o.targetPath||e.source||"sandbox",prompt:e.message})})).json();r.success?t.showToast("\u{1F680} Autonomous Patch deployed successfully! Vercel is building...","success"):t.showToast(`\u274C Autonomous deploy failed: ${r.error}`,"error")}catch(s){t.showToast(`\u274C Autonomous patch failed: ${s.message}`,"error")}}mountDashboard(e){T.mount(e)}shutdown(){this.interceptor.stop(),this.initialized=!1}get getConfig(){return this.config}get patcher(){return C}get emailer(){return v}get widget(){return y}get dashboard(){return T}},H=new L;typeof window<"u"&&window.AUTOHEAL_SITE_ID&&setTimeout(()=>{H.init({})},100);return j(R);})();
