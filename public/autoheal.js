...
===================================================
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let e=document.getElementById("ah-feature-view"),t=document.getElementById("ah-feature-footer"),s=document.getElementById("ah-settings-view"),a=document.getElementById("ah-settings-footer"),i=!1;document.getElementById("ah-settings-btn")?.addEventListener("click",async()=>{if(i=!i,i){e.style.display="none",t.style.display="none",s.style.display="block",a.style.display="flex";try{let r=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",c=window.AUTOHEAL_SITE_ID||window.location.host,d=await(await fetch(`${r}/api/settings`,{headers:{"x-site-id":c}})).json();d.groqKey&&(document.getElementById("ah-groq-key-input").value=d.groqKey)}catch(r){console.warn("AutoHeal: Could not fetch settings",r)}}else e.style.display="block",t.style.display="flex",s.style.display="none",a.style.display="none"}),document.getElementById("ah-save-settings-btn")?.addEventListener("click",async()=>{let r=document.getElementById("ah-groq-key-input").value.trim();if(!r)return;let c=window.AUTOHEAL_ENDPOINT||"http://localhost:3001",g=window.AUTOHEAL_SITE_ID||window.location.host,d=document.getElementById("ah-settings-status");d.textContent="Saving...";try{(await(await fetch(`${c}/api/settings`,{method:"POST",headers:{"Content-Type":"application/json","x-site-id":g},body:JSON.stringify({settings:{groqKey:r,modelProvider:"groq"}})})).json()).success?d.textContent="Settings saved successfully! \u2705":d.textContent="Error saving settings."}catch{d.textContent="Failed to connect to Master Server."}});let o=document.getElementById("ah-build-btn"),n=document.getElementById("ah-feature-prompt");o.addEventListener("click",()=>{let r=n.value.trim();if(!r)return;n.disabled=!0,o.disabled=!0,o.classList.add("disabled");let c=document.getElementById("ah-btn-loader");c&&(c.style.display="inline-block");let g=document.getElementById("ah-btn-text");g&&(g.textContent="BUILDING...");let d=document.getElementById("ah-diag-console");d&&(d.style.display="block");let p={id:"feature_"+Date.now(),type:"feature",message:r,timestamp:new Date().toISOString(),source:window.location.pathname==="/"?"src/App.jsx":window.location.pathname};this.runDiagnosticEngine(p,o)})}openDiagnosticModal(e,t){if(!this.container)return;t?(document.body.classList.add("ah-blur-active"),this.container.classList.add("ah-hard-crash")):this.container.classList.remove("ah-hard-crash"),this.container.style.display="flex";let s=new Date(e.timestamp).toLocaleTimeString();this.container.innerHTML=`
      <div class="ah-diag-modal">
        <div class="ah-diag-header">
          <div class="ah-diag-title">
    ...
    `,document.getElementById("ah-close-modal-btn")?.addEventListener("click",()=>this.closeDiagnosticModal()),document.getElementById("ah-ignore-btn")?.addEventListener("click",()=>this.closeDiagnosticModal());let a=document.getElementById("ah-patch-btn");this.runDiagnosticEngine(e,a)}async runDiagnosticEngine(e,t){let s=document.getElementById("ah-diag-console"),a=document.getElementById("ah-footer-status"),i=document.getElementById("ah-scanner-box"),o=document.getElementById("ah-patch-box"),n=document.getElementById("ah-diff-box");if(!s)return;let r=(p,m="default")=>{let f=document.createElement("div");f.className=`ah-console-line ${m}`,f.textContent=`> ${p}`,s.appendChild(f),s.scrollTop=s.scrollHeight};await this.delay(1e3),r("Analyzing stack trace patterns...","info"),a&&(a.textContent="Analyzing source-code stack trace..."),await this.delay(1200),r("Extracting code context around error location...","info"),await this.delay(1e3),r("Consulting AI LLM healing patterns...","comment"),a&&(a.textContent="Generating surgical repair code...");let c=!1,g="",d="";if(this.onHealHandler)try{let p=await this.onHealHandler(e);c=p.success,g=p.diffCode,d=p.explanation||""}catch{r("Failed to contact AI Healer Agent.","error")}if(await this.delay(800),c&&g){if(r("Surgical fix generated successfully!","success"),r("Ready to hot-patch runtime environment.","success"),a&&(a.textContent="Patch compiled successfully!"),i&&(i.style.display="none"),o&&(o.style.display="block"),n&&(n.innerHTML=this.renderDiff(g)),t){t.disabled=!1,t.classList.remove("disabled");let p=document.getElementById("ah-btn-loader");p&&(p.style.display="none");let m=document.getElementById("ah-btn-text");m&&(m.textContent="APPLY LIVE PATCH \u{1FA7A}"),t.onclick=async()=>{t.disabled=!0,t.classList.add("disabled"),p&&(p.style.display="inline-block"),m&&(m.textContent="Applying Patch..."),a&&(a.textContent="Injecting runtime hot-patch..."),await this.delay(1200),this.currentErrors=this.currentErrors.filter(f=>f.id!==e.id),this.updateBadgeCount(),this.showToast("\u{1F680} System Healed Successfully!","success"),this.closeDiagnosticModal(),r("Hot-patch successfully applied! Site recovered.","success")}}}else if(r(`AI agent failed: ${d||"Could not determine a safe patch."}`,"error"),a&&(a.textContent="Healing failed. Manual debug required."),t){t.disabled=!1,t.classList.remove("disabled");let p=document.getElementById("ah-btn-loader");p&&(p.style.display="none");let m=document.getElementById("ah-btn-text");m&&(m.textContent="Unable to heal")}}renderDiff(e){return e.split(`
